...
int main() {
    ...
    { // test quaternion->rotation matrix
        ...
        assert((M1-R).norm()<DOUBLE_NEAR_THRESHOLD);
    }
    ...
    { // test quaternion->rotation matrix
        ...
        assert((M2-R).norm()<DOUBLE_NEAR_THRESHOLD);
    }

    { // test multiplication of quaternions
        Quaternion q3 = q2*q1;
        mat<3,3> R = q3.rotation_matrix();
        assert((M2*M1-R).norm()<DOUBLE_NEAR_THRESHOLD);
    }
    ...
    { // test quaternion->Euler angles
        ...
        mat<3,3> R = q2.rotation_matrix();
        assert((M-R).norm()<DOUBLE_NEAR_THRESHOLD);
    }
