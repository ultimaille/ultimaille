...
namespace UM {
    namespace Linear {
        void LinExpr::compact() {
            ...
            for (int i = 0; i < size(); ) {
                data[s] = data[i++];
                while (i < size() && data[i].i == data[s].i)
                    data[s].a += data[i++].a;
        ...
        LinExpr operator+(const LinExpr& le1, const LinExpr& le2) {
            LinExpr result;
            int i = 0, j = 0;
            while (i < (int)le1.size() && j < (int)le2.size()) {
            ...
            result.data.reserve(result.size() + le1.size() + le2.size() - (i + j));
            result.data.insert(result.end(), le1.begin() + i, le1.end());
            result.data.insert(result.end(), le2.begin() + j, le2.end());
            return result;
        }
        ...
        LinExpr operator-(const LinExpr& le1, const LinExpr& le2) {
            ...
            result.data.reserve(result.size() + le1.size() + le2.size() - (i + j));
            result.data.insert(result.end(), le1.begin() + i, le1.end());
            while (j < (int)le2.size()) result.data.push_back(-le2.data[j++]);
            return result;
        }
