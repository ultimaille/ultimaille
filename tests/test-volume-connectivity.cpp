...
#define FOR(i, n) for(int i = 0; i < (int) n; i++)
    ...
    void test_volume(Volume& m) {
...

        {
            ...
            for (auto c : m.iter_cells()) FOR(lv, c.nverts())nb_neig_cells[c.vertex(lv)]++;
            for (auto h : m.iter_halfedges()) nb_neig_halfedges[h.from()]++;
            for (auto v : m.iter_vertices())
                if (3 * nb_neig_cells[v] != nb_neig_halfedges[v]) coherent_number_adjacent_cells_and_halfedges = false;
    ...
    void split_hex(Hexahedra& hex) {
        ...
        FOR(v, hex.points.size()) tokill[v] = (old2new[v] != v);
    ...
    void hex_grid(Hexahedra& m, int n) {
        ...
        int n2 = n1 * n1;
        FOR(i, n1 * n1 * n1) m.points[i] = (1. / double(n1)) * vec3(i / n2, (i - (i / n2) * n2) / n1, i % n1);
